...
```{r table: data dictionary, echo=TRUE, results='hide', tidy=TRUE, tidy.opts=list(width.cutoff=60)}
...
``` {r, Binary columns For easier visualizations later, include=TRUE}
...
```{r regression_prep, message=FALSE, warning=FALSE, include=TRUE, results='hide'}
library(lubridate)
...
## Regression Summary
```{r regression_model, message=FALSE, warning=FALSE, echo=TRUE}
...
## Time Series Plot
```{r time_series_plot, message=FALSE, warning=FALSE, echo=TRUE}
library(ggplot2)
...
\newpage
## Seasonality Plot
```{r seasonality_plot, message=FALSE, warning=FALSE, echo=TRUE}
...
```{r forecast_plot, message=FALSE, warning=FALSE, echo=TRUE}
